...
- Web / 移动端复习页增加「复习概览 / 学习周报」栏目，周报支持历史分页与栏目内阅读，首页周报卡跳入此处。
- 文档保留底层内容存储，增加 `kind` 和 `reportWeekStart`；普通文档列表在服务端排除周报，`GET /api/reports/weekly` 返回当前用户按周期排序的历史周报。
- 迁移 `0024_military_switch` 从 mastery memory 回填历史类型与周期，旧的重复生成记录按标题周期回填，兼容跨年与延迟生成。现有文档链接仍可访问。
- 验证：Web build；Web / server / mobile typecheck；Web 110、server 371、mobile 6 项测试通过。迁移以临时表事务回滚验证跨年、改名、普通文档三种情况；Web 实测列表、首页入口、正文与窄屏布局，移动端尚未真机验证。
