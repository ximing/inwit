...
const ReviewPageContent = observer(function ReviewPageContent() {
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if (event.key === '1') {
        event.preventDefault();
        void service.grade('forgot');
        return;
      }
      if (event.key === '2') {
        event.preventDefault();
        void service.grade('fuzzy');
        return;
      }
      if (event.key === '3') {
        event.preventDefault();
        void service.grade('remembered');
      }
    };
  ...
  return (
    <div className="hub">
      ...
      <div className="streak-hero">
        ...
        <div>
          {streak === 0 ? (
            <div className="streak-num">今天开始第一天</div>
          ) : (
            <div className="streak-num">
              {streak} <small>天连续复习</small>
            </div>
          )}
          <div className="streak-sub">最长纪录 {longest} 天 · 保持住，别断。</div>
        </div>
      ...
      <div className="hub-grid">
        <div className="hub-stat">
          ...
          <div className="hub-stat-label">总卡片</div>
        </div>
        <div className="hub-stat">
          <div className="hub-stat-num">
          ...
          <div className="hub-stat-label">已掌握（间隔 ≥ 21 天）</div>
        </div>
        <div className="hub-stat">
          <div className="hub-stat-num">
            {retention == null ? (
          ...
          <div className="hub-stat-label">近 7 天想起率</div>
        </div>
        <div className="hub-stat">
          <div className="hub-stat-num">
            {stats?.reviews7d ?? 0}
          ...
          <div className="hub-stat-label">近 7 天复习次数</div>
        </div>
      </div>

      <div className="hub-panel">
        <div className="hub-panel-head">
          <span className="hub-panel-title">近 7 天表现</span>
          <span className="hub-panel-sub">每天刷了多少、想起来多少</span>
        </div>
        <WeekBars daily={stats?.daily ?? []} />
      ...
      <div className="hub-panel">
        <div className="hub-panel-head">
          <span className="hub-panel-title">未来 7 天到期</span>
          <span className="hub-panel-sub">提前知道哪天的复习量大</span>
        </div>
        <ForecastBars forecast={stats?.forecast ?? []} />
      </div>
...
function WeekBars({ daily }: { daily: ReviewDailyBucket[] }) {
  const max = Math.max(1, ...daily.map(dayTotal));
  return (
    <div className="week-bars">
      {daily.map((row) => {
        const remembered = (row.remembered / max) * 100;
        const fuzzy = (row.fuzzy / max) * 100;
        const forgot = (row.forgot / max) * 100;
        return (
          <div className="wb-col" key={row.date}>
            <div className="wb-stack">
              {row.remembered > 0 ? (
                <i className="wb-good" style={{ height: `${String(remembered)}%` }} />
              ) : null}
              {row.fuzzy > 0 ? (
                <i className="wb-hazy" style={{ height: `${String(fuzzy)}%` }} />
              ) : null}
              {row.forgot > 0 ? (
                <i className="wb-bad" style={{ height: `${String(forgot)}%` }} />
              ) : null}
            </div>
            <span className="wb-day">{weekdayShort(row.date)}</span>
          </div>
        );
      })}
    </div>
...
function ForecastBars({ forecast }: { forecast: ReviewForecastBucket[] }) {
  const max = Math.max(1, ...forecast.map((row) => row.count));
  return (
    <div className="forecast">
      {forecast.map((row, index) => {
        const height = `${String((row.count / max) * 100)}%`;
        return (
          <div className="fc-col" key={row.date}>
            <div className="fc-bar" style={{ height }}>
              {row.count > 0 ? <b>{row.count}</b> : null}
            </div>
            <span className="fc-day">{forecastLabel(row.date, index)}</span>
          </div>
        );
      })}
    </div>
...
const SessionPane = observer(function SessionPane() {
  ...
  const progressStyle = { width: `${String(service.progressPct)}%` } satisfies CSSProperties;
  ...
  return (
    <div className="review-stage">
      <div className="review-head">
        ...
        <span className="review-title">今日复习</span>
      ...
      {complete || !current ? (
        <div className="review-done">
          ...
          <p>
            明天到期 <b>{service.tomorrowDue}</b> 张
          </p>
        ...
        <div className="flip-zone">
          ...
          >
            <div className="flip-face flip-front">
              <span className="flip-kicker">先自己想</span>
              {imageUrl ? (
                <img
            ...
            <div className="flip-face flip-back">
              <span className="flip-kicker ans">答案</span>
          ...
          <div className={service.flipped ? 'grade-row is-show' : 'grade-row'}>
            <GradeButton
              ...
              disabled={grading || !service.flipped}
      ...
      {complete ? null : <div className="review-foot-tip">先自己想，想起来了再翻。不要偷看。</div>}
    </div>
  );
});
