...
function mimeFromUri(uri: string, mimeType?: string | null): string | null {
  if (mimeType) return normalizeAvatarMime(mimeType);
...
export class SettingsService extends Service {
  ...
  async uploadAvatar(asset: AvatarPick): Promise<void> {
    this.avatarError = null;
    const contentType = mimeFromUri(asset.uri, asset.mimeType);
    if (!contentType) {
    ...
    try {
      ...
      const put = await FileSystem.uploadAsync(uploadUrl, asset.uri, {
        httpMethod: 'PUT',
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      this.error = errorMessage(err, '加载设置失败');
    }
  }
  ...
  async add(): Promise<void> {
    ...
    try {
      ...
      this.configs = [
        created,
        ...this.configs.map((item) => (created.isDefault ? { ...item, isDefault: false } : item)),
      ];
      this.apiKey = '';
      this.baseUrl = '';
