...
export function usageSwatchColor(type: string, colors: ColorTokens): string {
  if (type === 'digest') return colors.accent;
  if (type === 'chat') return colors.gold;
  if (type === 'embed' || type === 'rerank') return colors.ink3;
  return colors.green;
}
...
export class JobsService extends Service {
  ...
  tickElapsed(): void {
    if (!this.pollingAllowed || !this.queue || this.queue.running.length === 0) {
      this.stopTick();
      return;
    }
    this.queue = {
  ...
  syncTick(): void {
    if (this.pollingAllowed && this.running.length > 0) this.startTick();
    else this.stopTick();
  }
