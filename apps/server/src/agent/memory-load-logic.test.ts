...
describe('keepUuidIds', () => {
  it('keeps order and drops duplicates and non-uuids without capping', () => {
    expect(keepUuidIds([C2, 'nope', C2, C1.toUpperCase(), C3, C4, ''])).toEqual([
      C2,
      C1.toUpperCase(),
      C3,
      C4,
    ]);
    expect(keepUuidIds([C1, C1])).toEqual([C1]);
    expect(keepUuidIds([C1, C1.toUpperCase()])).toEqual([C1]);
    expect(keepUuidIds(['not-a-uuid', 12, null])).toEqual([]);
