...
// Verified against a live call (vital, 2026-09): output.results[] carries
...
function parseRerankResponse(json: unknown): ParsedRerankResponse {
  ...
  const usage = root.usage as { total_tokens?: unknown; prompt_tokens?: unknown } | undefined;
  const promptTokens =
  ...
  return {
    async rerankTexts(query, documents, topN, opts) {
      ...
      return withRetrievalTelemetry(
        { userId: opts?.userId, capability: 'rerank', model: options.model },
        async () => {
