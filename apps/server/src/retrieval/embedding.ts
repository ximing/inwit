...
// Verified against a live call (vital, 2026-09): output.embeddings[] carries
...
export function createEmbeddingClient(options: EmbeddingClientOptions): EmbeddingClient {
  ...
  async function embedBatch(texts: string[]): Promise<ParsedEmbeddingResponse> {
    ...
    const parsed = parseEmbeddingResponse(res.json, texts.length);
    for (const vector of parsed.vectors) {
      if (vector.length !== options.dimensions) {
        throw new RetrievalError(
          'BAD_RESPONSE',
          `embedding dimension mismatch: expected ${String(options.dimensions)}, got ${String(vector.length)}`,
        );
      }
    }
    return parsed;
  ...
  return {
    async embedTexts(texts, opts) {
      ...
      return withRetrievalTelemetry(
        { userId: opts?.userId, capability: 'embed', model: options.model },
        async () => {
